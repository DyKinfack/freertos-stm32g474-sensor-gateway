...
which passes a pre-initialized Data\_t structure identifying the sensor type.
...
* Pack the measured value into a **Data\_t** structure containing sensor value and source identifier
* Send the structure to the shared **Queue** using **xQueueSend()**
* Set the corresponding Watchdog Flag (**watchdogFlag\_Humidity** or **watchdogFlag\_Pressure**) on successful send
...
* Receive Data\_t structures from the Queue using **xQueueReceive(portMAX\_DELAY)**
* Determine the correct threshold based on sensor type (**HUMIDITY\_THRESHOLD** or **PRESSURE\_THRESHOLD**)
...
* Output the buffer content via **printf() → routed through \_\_io\_putchar() → LPUART1**
* Release the Mutex
...
* Block on **xSemaphoreTake(binarySemaphrHandle, portMAX\_DELAY)**
...
* Every 5 seconds: check **watchdogFlag\_Humidity** and **watchdogFlag\_Pressure**
...
**Watchdog Flags**		volatile uint8\_t flags set by sensor Tasks
